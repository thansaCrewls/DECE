...
class DiscordBot {
    ...
    async deleteMessage(channelId, messageId) {
        await axios.delete(`${this.baseUrl}/channels/${channelId}/messages/${messageId}`, { headers: this.headers });
    }
}
...
function displayIntro() {
    console.log(chalk.cyan(figlet.textSync('DISCORD-BOT', { horizontalLayout: 'Standard' })));
    console.log(chalk.green('Sharing Airdrop Community\n'));
}
            ...
            try {
                ...
                for (const channel of config.channel_id) {
                    ...
                    if (response.content) {
                        ...
                        // Immediately delete the message after sending it
                        await bot.deleteMessage(channel, response.id);
                        console.log(chalk.blue(`[INFO] [${username}] => Deleted message in Channel ${channel}: ${customMessage}`));
                    }
